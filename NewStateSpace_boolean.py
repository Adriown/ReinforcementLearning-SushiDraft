...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 19 12:34:08 2018
...
#                                    if other[i] < played[i]:
#                                        other[i] = -1
#                                    elif other[i] == played[i]:
#                                        other[i] = 0
#                                    else:
#                                        other[i] = 1
                                if other_bool not in other_bool_unique:
                                    possStates.append([hand,played,other_bool])
                                    other_bool_unique.append(other_bool)
                                #print([hand,played,other])
                                
 

len(possStates)
print(possStates[296500:-1])

#possStates_df.columns = ['hand', 'our_played','others']
#for i, row in possStates_df.iterrows():
#    for i in range(len(row['our_played'])):
#        if row['our_played'][i] > row['others'][i]:
#            row['others'][i] = -1
#            #print(row['others'][i])
#        elif row['our_played'][i] == row['others'][i]:
#            row['others'][i] = 0
#            #print(row['others'][i])
#        else:
#            row['others'][i] = 1
...
import pandas as pd
possStates_df = pd.DataFrame(possStates)
possStates_df.to_csv("/Users/kia/Desktop/Reinforcement Learning/ReinforcementLearning-SushiDraft/statespace_boolean.csv")
